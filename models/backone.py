...
class ImageBackBone(nn.Module):
    ...
    def __init__(self, block, num_block, use_bn=True):
        ...
        self.deconv1 = nn.ConvTranspose2d(
            384, 256, kernel_size=3, stride=2, padding=1, output_padding=1
        )
...
class LiDAR(nn.Module):
    ...
    def __init__(self, block, num_block, geom, use_bn=True):
        ...
        self.deconv2 = nn.ConvTranspose2d(
            128, 96, kernel_size=3, stride=2, padding=1, output_padding=(1, 0)
        )
        # MLP
        # 2096 = 16*(128+3)
        self.image_feature_dim = self.geom["knn_shape"][2] * (128 + 3)
        self.squeeze_fusion = nn.Conv2d(
            self.image_feature_dim, 256, kernel_size=1, stride=1
        )
    ...
    def forward(self, x, y, x2y, pc_diff):
        ...
        """ c1 is the bev data input into the network and the result obtained after two layers of convolution calculations. """
        if self.fusion:
    ...
    def bev_image_fusion(self, y, x2y, pc_diff):
        ...
        if True:
            ...
            """ y bev shape now is (bs,400,350,16,-1,128) 128 should be calculated following the image feature channel """
